...
required_packages <- c(
  ...
  "httr",          # HTTP requests for PubMed/Scopus APIs
  "xml2",          # XML parsing for PubMed responses
...
**Result**: List of thirteen required packages defined.
...
Verify that API keys for Gemini, PubMed, and Scopus are configured.
...
cat("\nAPI Key Status:\n")
cat(rep("-", 40), "\n", sep = "")
cat("GOOGLE_API_KEY:  ", ifelse(google_set, "✓ SET", "✗ NOT SET"), "\n")
cat("PUB_MED_API_KEY: ", ifelse(pubmed_set, "✓ SET", "✗ NOT SET"), " (optional)\n")
cat("SCOPUS_API_KEY:  ", ifelse(scopus_set, "✓ SET", "✗ NOT SET"), "\n")

if (google_set) {
  cat("\nGemini API:\n")
  cat("  Key length:", nchar(google_key), "characters\n")
  cat("  Key preview:", substr(google_key, 1, 10), "...\n")
}

if (pubmed_set) {
  cat("\nPubMed API:\n")
  cat("  Key length:", nchar(pubmed_key), "characters\n")
  cat("  Key preview:", substr(pubmed_key, 1, 8), "...\n")
  cat("  Rate limit: 10 requests/second (with key)\n")
} else {
  cat("\nPubMed API: No key (3 requests/second without key)\n")
}

if (scopus_set) {
  cat("\nScopus API:\n")
  cat("  Key length:", nchar(scopus_key), "characters\n")
  cat("  Key preview:", substr(scopus_key, 1, 8), "...\n")
}

cat(rep("-", 40), "\n", sep = "")
cat("\nTo set up API keys, add to .env file:\n")
cat("  GOOGLE_API_KEY=your_google_key     (required for LLM extraction)\n")
cat("  PUB_MED_API_KEY=your_pubmed_key    (optional, increases rate limit)\n")
cat("  SCOPUS_API_KEY=your_scopus_key     (required for Scopus data fetching)\n")
```

**Result**: All API key configurations displayed.
...
cat("Google API:      ", ifelse(google_set, "✓ Configured", "✗ Not set"), "\n")
cat("PubMed API:      ", ifelse(pubmed_set, "✓ Configured", "○ Optional"), "\n")
cat("Scopus API:      ", ifelse(scopus_set, "✓ Configured", "✗ Not set"), "\n")
cat("Directories:     ✓ Ready\n")
cat("Functions:       ✓ Loaded\n")
cat(rep("-", 40), "\n", sep = "")

ready_for_extraction <- all(load_success) && google_set
ready_for_fetching <- all(load_success) && (pubmed_set || scopus_set)

if (ready_for_extraction) {
  cat("\n✓ Environment ready for LLM extraction!\n")
  cat("  Next: Run notebook 0010_test_gemini_api.Rmd\n")
}

if (ready_for_fetching) {
  cat("\n✓ Environment ready for data fetching!\n")
  cat("  Next: Run notebook 0090_fetch_pubmed_data.Rmd or 0095_fetch_scopus_data.Rmd\n")
}

if (!ready_for_extraction && !ready_for_fetching) {
  cat("\n⚠ Setup items need attention:\n")
  if (!all(load_success)) {
    cat("  - Fix package loading errors\n")
  }
  if (!google_set) {
    cat("  - Configure GOOGLE_API_KEY for LLM extraction\n")
  }
  if (!pubmed_set && !scopus_set) {
    cat("  - Configure PUB_MED_API_KEY or SCOPUS_API_KEY for data fetching\n")
  }
}
```
