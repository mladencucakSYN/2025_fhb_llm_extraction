...
if (api_key_present) {
  ...
  cat("5. Add to .env: ELSEVIER_API_KEY=your_key_here\n")
...
```{r build-query}
# Query components (same logic as PubMed, different syntax)
fusarium_terms <- "(fusarium OR \"fusarium graminearum\" OR \"fusarium head blight\" OR FHB)"
crop_terms <- "(wheat OR barley OR maize OR corn OR cereal OR grain)"
abiotic_terms <- "(temperature OR water OR moisture OR humidity OR climate OR environment OR abiotic OR stress)"

# Combine and wrap in Scopus TITLE-ABS-KEY syntax
fusarium_query <- paste0(
  "TITLE-ABS-KEY(",
  fusarium_terms, " AND ", crop_terms, " AND ", abiotic_terms,
  ")"
)

cat("Fusarium Ecophysiology Query (Scopus syntax):\n")
cat(rep("=", 70), "\n", sep = "")
cat(strwrap(fusarium_query, width = 70), sep = "\n")
cat(rep("=", 70), "\n", sep = "")
```
...
if (file.exists(pubmed_file) && file.exists(scopus_file)) {
  ...
  # Save merged dataset
  merged_file <- file.path(data_dir, "fusarium_combined_pubmed_scopus.csv")
  write_csv(combined_dedupe, merged_file)
  cat("\nSaved merged data to:", merged_file, "\n")

  rds_file <- file.path(data_dir, "fusarium_combined_pubmed_scopus.rds")
